...
class AulasController extends Controller
{
     ...
     */
     ...
     public function store(Request $request)
     {
         ...
         $professor = Professor::where('professor_id', $request->professor_id)->exists();
 
         $disciplina = Disciplina::where('disciplinas_id', $request->disciplinas_id)->exists();
 
         if (!$disciplina) {
             Log::debug('Disciplina não encontrada');
             return response()->json([
                 'message' => 'Disciplina não foi encontrada'
             ], 400);
         }
 
         if (!$professor) {
             Log::debug('Professor não encontrada');
             return response()->json([
